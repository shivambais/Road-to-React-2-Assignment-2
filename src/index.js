...
async function sortFun(){
    // let sortVar= document.querySelector("#sort").value;
    if(query!=null){
        let sortVar= this.value;
        let url= `https://api.unsplash.com/search/photos/?query=${query}&per_page=20&order_by=${sortVar}&client_id=5z9p4Ky25UYCO4DrHAT8jEYWS_NrUcaL_mRmuwOvx_8`
...
async function filterFun(){
    ...
    if(query!=null){
        let filterVar= this.value;
        let url= `https://api.unsplash.com/search/photos/?query=${query}&per_page=20&color=${filterVar}&client_id=5z9p4Ky25UYCO4DrHAT8jEYWS_NrUcaL_mRmuwOvx_8`
...
document.querySelector("#filter").addEventListener("change",filterFun);